...
$groupLevelsList = [];
$currentLevel = trim((string)($player["group_level"] ?? ""));
$playerSpecificLevel = trim((string)($player["player_level"] ?? ""));
try {
    $stmt = $pdo->prepare(
        "SELECT DISTINCT group_level FROM sports_groups
         WHERE game_id = ? AND group_level <> '' ORDER BY group_level ASC"
    );
    $stmt->execute([$playerGameId]);
    foreach ($stmt->fetchAll() as $r) {
        $groupLevelsList[] = (string)$r["group_level"];
    }
} catch (PDOException $ignored) {}
    ...
    <div class="pp-body">
        ...
        <aside class="pp-sidebar" id="ppSidebar">
            ...
            <a href="?section=levels" class="pp-side-item <?php echo $activeSection === 'levels' ? 'active' : ''; ?>">
                <span class="pp-side-icon">🏆</span><span class="pp-side-label">مستويات المجموعة</span>
            </a>
        ...
        <main class="pp-main">
            <div class="pp-main-inner">
                ...
                <?php elseif ($activeSection === 'levels'): ?>
                    <div class="pp-section">
                        <div class="pp-section-h">🏆 مستويات المجموعة - <?php echo pportEsc($player["game_name"] ?? ""); ?></div>
                                ...
                                <?php $i = 1; foreach ($groupLevelsList as $lvl): ?>
                                    ...
                                    <div class="pp-level <?php echo $isActive ? 'active' : ''; ?>">
                                        ...
                                        <div>
                                            ...
                                            <div class="pp-level-sub"><?php echo $isActive ? '🌟 مستواك الحالي' : 'مستوى'; ?></div>
                                        </div>
                                    </div>
                                <?php $i++; endforeach; ?>
                            </div>
