...
$trainerId = (int)$_SESSION["trainer_portal_id"];
$trainerGameId = (int)$_SESSION["trainer_portal_game_id"];
$siteName = (string)($_SESSION["trainer_portal_site_name"] ?? "أكاديمية رياضية");
$siteLogo = (string)($_SESSION["trainer_portal_site_logo"] ?? "assets/images/logo.png");

$trainer = null;
try {
    $trainerStmt = $pdo->prepare(
        "SELECT t.id, t.name, t.phone, t.barcode, t.salary, t.game_id,
...
if (!$trainer) {
    header("Location: trainer_portal_logout.php");
    exit;
}
...
function portalFormatAttendanceStatus($status) {
    $map = [
...
.portal-tab-btn:hover {
    background: rgba(47, 91, 234, 0.07);
    color: var(--primary);
}
.portal-tab-btn.active {
    background: var(--primary);
    color: #fff;
    box-shadow: 0 2px 8px rgba(47, 91, 234, 0.3);
}
.portal-tab-pane {
    display: none;
}
    ...
    <div class="portal-topbar">
        <div class="portal-topbar-brand">
            ...
            <div>
                ...
                <span class="portal-topbar-badge">🏋️ بوابة المدربين</span>
            </div>
        </div>
        <div class="portal-topbar-actions">
            <div class="portal-topbar-user">
                <span>👤</span>
                <span><?php echo htmlspecialchars($trainer["name"], ENT_QUOTES, "UTF-8"); ?></span>
            </div>
            ...
            <a href="trainer_portal_logout.php" class="portal-logout-btn">🚪 خروج</a>
        </div>
    </div>
    ...
    <div class="portal-main">
        ...
        <div class="portal-stats-grid">
            <div class="portal-stat-card">
                ...
                <div class="portal-stat-value"><?php echo portalFormatCurrency($trainer["salary"]); ?></div>
            ...
            <div class="portal-stat-card">
                <span class="portal-stat-icon">💳</span>
                <div class="portal-stat-value"><?php echo portalFormatCurrency($loansTotal); ?></div>
                <div class="portal-stat-label">إجمالي السلف</div>
            </div>
            <div class="portal-stat-card">
                <span class="portal-stat-icon">📉</span>
                <div class="portal-stat-value"><?php echo portalFormatCurrency($deductionsTotal); ?></div>
            ...
            <div class="portal-section-card">
                ...
                <div class="portal-info-grid">
                    <div class="portal-info-item">
                        ...
                        <div class="portal-info-value"><?php echo htmlspecialchars($trainer["name"], ENT_QUOTES, "UTF-8"); ?></div>
                    </div>
                    <div class="portal-info-item">
                        <div class="portal-info-label">رقم الهاتف</div>
                        <div class="portal-info-value"><?php echo htmlspecialchars($trainer["phone"], ENT_QUOTES, "UTF-8"); ?></div>
                    </div>
                    <div class="portal-info-item" style="grid-column:1/-1; text-align:center; background:#ffffff;">
                        ...
                        <div class="portal-info-value" style="display:flex; flex-direction:column; align-items:center; gap:10px; padding:14px 6px;">
                            <svg id="trainerPortalBarcode" style="max-width:100%; height:auto;"></svg>
                            <div style="display:inline-flex; align-items:center; gap:10px; padding:8px 16px; background:#f1f5f9; color:#0f172a; border-radius:10px; font-weight:800; letter-spacing:.04em;">
                                <span>📟</span>
                                <span dir="ltr"><?php echo htmlspecialchars((string)($trainer["barcode"] !== "" ? $trainer["barcode"] : "—"), ENT_QUOTES, "UTF-8"); ?></span>
                            </div>
                        </div>
                    </div>
                    <div class="portal-info-item">
                        <div class="portal-info-label">اللعبة / النشاط</div>
                        <div class="portal-info-value"><?php echo htmlspecialchars((string)($trainer["game_name"] ?? "—"), ENT_QUOTES, "UTF-8"); ?></div>
                    </div>
                    <div class="portal-info-item">
                        <div class="portal-info-label">الراتب الأساسي</div>
                        <div class="portal-info-value"><?php echo portalFormatCurrency($trainer["salary"]); ?></div>
                    </div>
                    <div class="portal-info-item">
                        <div class="portal-info-label">وقت الحضور المقرر</div>
                        <div class="portal-info-value"><?php echo portalFormatTime($trainer["attendance_time"]); ?></div>
                    </div>
                    <div class="portal-info-item">
                        <div class="portal-info-label">وقت الانصراف المقرر</div>
                        <div class="portal-info-value"><?php echo portalFormatTime($trainer["departure_time"]); ?></div>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <table class="portal-table">
                            ...
                            <tbody>
                                <?php foreach ($attendanceRows as $ar): ?>
                                    <?php $statusInfo = portalFormatAttendanceStatus($ar["day_status"]); ?>
                                    <tr>
                                        <td><?php echo htmlspecialchars(portalFormatDate($ar["attendance_date"]), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td><?php echo htmlspecialchars(portalFormatTime($ar["scheduled_attendance_time"]), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td><?php echo htmlspecialchars(portalFormatDatetime((string)($ar["attendance_at"] ?? "")), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td><?php echo htmlspecialchars(portalFormatTime($ar["scheduled_departure_time"]), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td><?php echo htmlspecialchars(portalFormatDatetime((string)($ar["departure_at"] ?? "")), ENT_QUOTES, "UTF-8"); ?></td>
            ...
            <div class="portal-section-card">
                ...
                <?php else: ?>
                        ...
                        <table class="portal-table">
                            ...
                            <tbody>
                                <?php foreach ($deductionRows as $dr): ?>
                                    <tr>
                                        <td><?php echo htmlspecialchars(portalFormatDate($dr["deduction_date"]), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td><span class="badge-danger"><?php echo htmlspecialchars(portalFormatCurrency($dr["amount"]), ENT_QUOTES, "UTF-8"); ?></span></td>
                                        <td><?php echo htmlspecialchars((string)($dr["reason"] ?? "—"), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td><?php echo htmlspecialchars(portalFormatDatetime((string)($dr["created_at"] ?? "")), ENT_QUOTES, "UTF-8"); ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                    ...
                    <div style="margin-top:16px; text-align:left; font-size:0.9rem; font-weight:800; color:var(--danger);">
                        الإجمالي: <?php echo portalFormatCurrency($deductionsTotal); ?>
                    </div>
                <?php endif; ?>
            </div>
        </div>
            ...
            <div class="portal-section-card">
                ...
                <?php else: ?>
                        ...
                        <table class="portal-table">
                            ...
                            <tbody>
                                <?php foreach ($loanRows as $lr): ?>
                                    <tr>
                                        <td><?php echo htmlspecialchars(portalFormatDate($lr["loan_date"]), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td><span class="badge-warning"><?php echo htmlspecialchars(portalFormatCurrency($lr["amount"]), ENT_QUOTES, "UTF-8"); ?></span></td>
                                        <td><?php echo htmlspecialchars(portalFormatDatetime((string)($lr["created_at"] ?? "")), ENT_QUOTES, "UTF-8"); ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                    ...
                    <div style="margin-top:16px; text-align:left; font-size:0.9rem; font-weight:800; color:var(--warning);">
                        الإجمالي: <?php echo portalFormatCurrency($loansTotal); ?>
                    </div>
                <?php endif; ?>
            </div>
        </div>
                        ...
                        <table class="portal-table">
                            ...
                            <tbody>
                                <?php foreach ($collectionRows as $cr): ?>
                                    <tr>
                                        <td><?php echo htmlspecialchars(portalFormatDate((string)($cr["salary_month"] ?? "")), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td><?php echo htmlspecialchars(portalFormatCurrency($cr["base_salary"]), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td><span class="badge-warning"><?php echo htmlspecialchars(portalFormatCurrency($cr["loans_total"]), ENT_QUOTES, "UTF-8"); ?></span></td>
                                        <td><span class="badge-danger"><?php echo htmlspecialchars(portalFormatCurrency($cr["deductions_total"]), ENT_QUOTES, "UTF-8"); ?></span></td>
                                        <td><span class="badge-success"><?php echo htmlspecialchars(portalFormatCurrency($cr["net_salary"]), ENT_QUOTES, "UTF-8"); ?></span></td>
                                        <td>
                                            <?php
                                            $paid = $cr["actual_paid_amount"] ?? null;
                                            echo $paid !== null
                                                ? htmlspecialchars(portalFormatCurrency($paid), ENT_QUOTES, "UTF-8")
                                                : "—";
                                            ?>
                                        </td>
                                        <td><?php echo (int)($cr["attendance_days"] ?? 0); ?></td>
                                        <td><?php echo (int)($cr["absent_days"] ?? 0); ?></td>
                                        <td><?php echo htmlspecialchars(portalFormatDatetime((string)($cr["paid_at"] ?? "")), ENT_QUOTES, "UTF-8"); ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                        ...
                        <div class="notif-card">
                            <div class="notif-header">
                                ...
                                <div class="notif-meta">
                                    ...
                                    <span class="notif-date"><?php echo htmlspecialchars(portalFormatDate($nr["display_date"]), ENT_QUOTES, "UTF-8"); ?></span>
                                </div>
                            </div>
...
(function () {
    var el = document.getElementById("trainerPortalBarcode");
    if (el && window.JsBarcode) {
        var code = <?php echo json_encode((string)($trainer["barcode"] ?? ""), JSON_UNESCAPED_UNICODE); ?>;
...
window.__PORTAL_SESSION_GUARD__ = {
    ...
    logoutUrl: "trainer_portal_logout.php"
};
</script>
