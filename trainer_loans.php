...
const TRAINER_LOAN_EMPTY_VALUE = "—";

requireAuthenticatedUser();
requireMenuAccess("trainers-loans");

function isValidTrainerLoanDate($date)
{
...
function formatTrainerLoanDateTimeValue($dateTimeString)
{
    ...
    if ($dateTimeString === "") {
        return TRAINER_LOAN_EMPTY_VALUE;
    }
    ...
    return formatTrainerLoanEgyptDateTimeLabel($dateTime);
}

function formatTrainerLoanAmountWithCurrency($amount)
{
    return number_format((float)$amount, 2) . " ج.م";
}

function normalizeTrainerLoanAmount($amount)
{
    return number_format((float)$amount, 2, ".", "");
}

function ensureTrainerLoansTable(PDO $pdo)
{
    $pdo->exec(
    ...
    $requiredColumns = [
        "trainer_name" => "ALTER TABLE trainer_loans ADD COLUMN trainer_name VARCHAR(150) NOT NULL DEFAULT '' AFTER trainer_id",
        "updated_by_user_id" => "ALTER TABLE trainer_loans ADD COLUMN updated_by_user_id INT(11) NULL DEFAULT NULL AFTER created_by_user_id",
        "created_by_user_id" => "ALTER TABLE trainer_loans ADD COLUMN created_by_user_id INT(11) NULL DEFAULT NULL AFTER loan_date",
        "loan_date" => "ALTER TABLE trainer_loans ADD COLUMN loan_date DATE NOT NULL AFTER amount",
    ];

    foreach ($requiredColumns as $columnName => $sql) {
        $columnStmt = $pdo->query("SHOW COLUMNS FROM trainer_loans LIKE " . $pdo->quote($columnName));
        if (!$columnStmt->fetch()) {
            $pdo->exec($sql);
        }
    }
...
function fetchTrainerLoanTrainers(PDO $pdo, $gameId)
{
    $stmt = $pdo->prepare(
        "SELECT id, name
         FROM trainers
...
function fetchTrainerLoanRecord(PDO $pdo, $loanId, $gameId)
{
    $stmt = $pdo->prepare(
        "SELECT id, trainer_id, trainer_name, amount, loan_date
         FROM trainer_loans
...
function fetchTrainerLoanRows(PDO $pdo, $gameId, $selectedDate, $trainerId = 0)
{
    $sql = "SELECT
            l.id,
            l.trainer_id,
            ...
            l.created_at,
            l.updated_at,
            COALESCE(t.name, l.trainer_name) AS display_trainer_name,
            COALESCE(created_user.username, " . $pdo->quote(TRAINER_LOAN_EMPTY_VALUE) . ") AS created_by_name,
            COALESCE(updated_user.username, " . $pdo->quote(TRAINER_LOAN_EMPTY_VALUE) . ") AS updated_by_name
        FROM trainer_loans l
        LEFT JOIN trainers t
            ON t.id = l.trainer_id
           AND t.game_id = l.game_id
...
function summarizeTrainerLoanRows(array $rows)
{
    $summary = [
        "count" => count($rows),
        "total_amount" => 0.0,
...
function escapeTrainerLoanXml($value)
{
    return htmlspecialchars((string)$value, ENT_XML1 | ENT_COMPAT, "UTF-8");
}

function buildTrainerLoanWorksheetCell($cellReference, $value, $styleId)
{
    return '<c r="' . $cellReference . '" s="' . $styleId . '" t="inlineStr"><is><t xml:space="preserve">' . escapeTrainerLoanXml($value) . '</t></is></c>';
}

function getTrainerLoanExcelColumnName($columnNumber)
{
...
function buildTrainerLoanWorksheetXml(array $headers, array $rows)
{
    ...
    foreach ($headers as $index => $header) {
        $xml .= buildTrainerLoanWorksheetCell(getTrainerLoanExcelColumnName($index + 1) . "1", $header, 1);
    }
    $xml .= '</row>';
    ...
    foreach ($rows as $rowIndex => $row) {
        ...
        foreach ($row as $cellIndex => $cellValue) {
            $xml .= buildTrainerLoanWorksheetCell(getTrainerLoanExcelColumnName($cellIndex + 1) . $excelRow, $cellValue, 2);
        }
        $xml .= '</row>';
    }
    ...
    $xml .= '</sheetData>';
    if (count($rows) > 0) {
        $xml .= '<autoFilter ref="A1:' . getTrainerLoanExcelColumnName(count($headers)) . (count($rows) + 1) . '"/>';
    }
    $xml .= '</worksheet>';
    return $xml;
}

function buildTrainerLoanStylesXml()
{
...
function exportTrainerLoansXlsx(array $records, $selectedDate, $currentGameName)
{
    if (!class_exists("ZipArchive")) {
        error_log("ZipArchive extension not available for trainer loans export.");
    ...
    foreach ($records as $index => $record) {
        $rows[] = [
            ...
            normalizeTrainerLoanAmount($record["amount"] ?? 0),
            formatTrainerLoanDateTimeValue($record["created_at"] ?? ""),
            (string)($record["created_by_name"] ?? TRAINER_LOAN_EMPTY_VALUE),
            (string)($record["updated_by_name"] ?? TRAINER_LOAN_EMPTY_VALUE),
        ];
    }

    $sheetXml = buildTrainerLoanWorksheetXml($headers, $rows);
    $stylesXml = buildTrainerLoanStylesXml();
    $timestamp = (new DateTimeImmutable("now", new DateTimeZone("Africa/Cairo")))->format("Y-m-d\\TH:i:sP");
    $tempFile = sys_get_temp_dir() . "/trainer-loans-" . uniqid("", true) . ".xlsx";
    ...
    $cleanupTempFile = static function () use ($tempFile, &$tempFileDeleted) {
        ...
        if (file_exists($tempFile) && !unlink($tempFile)) {
            error_log("Failed to delete temporary trainer loans XLSX file: " . $tempFile);
        }
    };
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <dc:title>سلف المدربين - ' . escapeTrainerLoanXml($currentGameName) . '</dc:title>
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
    <sheets>
        <sheet name="سلف المدربين" sheetId="1" r:id="rId1"/>
    </sheets>
</workbook>');
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
    ...
    $fileName = "trainer-loans-" . $selectedDate . ".xlsx";
...
function logTrainerLoanException(Throwable $throwable)
{
    error_log("Trainer loans page error: " . $throwable->getMessage());
}

if (!isset($_SESSION["trainer_loans_csrf_token"])) {
    $_SESSION["trainer_loans_csrf_token"] = bin2hex(random_bytes(32));
}

ensureTrainerLoansTable($pdo);
...
$egyptDateTimeLabel = formatTrainerLoanEgyptDateTimeLabel($egyptNow);

$selectedDate = trim((string)($_GET["date"] ?? $todayDate));
if (!isValidTrainerLoanDate($selectedDate)) {
    $selectedDate = $todayDate;
}

$trainers = fetchTrainerLoanTrainers($pdo, $currentGameId);
$trainerMap = [];
...
$flashSuccess = $_SESSION["trainer_loans_success"] ?? "";
unset($_SESSION["trainer_loans_success"]);
if ($flashSuccess !== "") {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!hash_equals($_SESSION["trainer_loans_csrf_token"], $csrfToken)) {
        ...
        if ($action === "save") {
            ...
            } elseif (!isValidTrainerLoanDate($formData["loan_date"])) {
                $error = "تاريخ السلفة غير صحيح.";
            }

            if ($error === "") {
                $existingLoan = $formData["id"] > 0 ? fetchTrainerLoanRecord($pdo, $formData["id"], $currentGameId) : null;
            ...
            if ($error === "") {
                ...
                $amountValue = normalizeTrainerLoanAmount($formData["amount"]);

                try {
                    if ($formData["id"] > 0) {
                        $updateStmt = $pdo->prepare(
                            "UPDATE trainer_loans
                             SET trainer_id = ?, trainer_name = ?, amount = ?, loan_date = ?, updated_by_user_id = ?
                             WHERE id = ? AND game_id = ?"
                        );
                        $updateStmt->execute([
                            $formData["trainer_id"],
                        ...
                        auditLogActivity($pdo, "update", "trainer_loans", $formData["id"], "سلف المدربين", "تعديل سلفة لـ: " . $trainerName . " بمبلغ " . $amountValue);
                        $_SESSION["trainer_loans_success"] = "تم تحديث السلفة.";
                    } else {
                        $insertStmt = $pdo->prepare(
                            "INSERT INTO trainer_loans (game_id, trainer_id, trainer_name, amount, loan_date, created_by_user_id, updated_by_user_id)
                             VALUES (?, ?, ?, ?, ?, ?, ?)"
                        );
                        $insertStmt->execute([
                            $currentGameId,
                        ...
                        auditLogActivity($pdo, "create", "trainer_loans", $newLoanId, "سلف المدربين", "إضافة سلفة لـ: " . $trainerName . " بمبلغ " . $amountValue);
                        $_SESSION["trainer_loans_success"] = "تم تسجيل السلفة.";
                    }

                    header("Location: trainer_loans.php?date=" . urlencode($formData["loan_date"]));
                    exit;
                } catch (Throwable $throwable) {
                    logTrainerLoanException($throwable);
        ...
        if ($action === "delete") {
            ...
            if (!isValidTrainerLoanDate($redirectDate)) {
            ...
            if (!$isManager) {
                ...
                $loanRecordToDelete = fetchTrainerLoanRecord($pdo, $loanId, $currentGameId);
                $deleteStmt = $pdo->prepare("DELETE FROM trainer_loans WHERE id = ? AND game_id = ?");
                ...
                try {
                    ...
                    if ($deleteStmt->rowCount() === 0) {
                        ...
                        $deletedLoanLabel = $loanRecordToDelete ? ((string)($loanRecordToDelete["trainer_name"] ?? "") . " بمبلغ " . (string)($loanRecordToDelete["amount"] ?? "")) : "";
                        auditLogActivity($pdo, "delete", "trainer_loans", $loanId, "سلف المدربين", "حذف سلفة: " . $deletedLoanLabel);
                        $_SESSION["trainer_loans_success"] = "تم حذف السلفة.";
                        header("Location: trainer_loans.php?date=" . urlencode($redirectDate));
                        exit;
                    }
                } catch (Throwable $throwable) {
                    logTrainerLoanException($throwable);
...
if ($isManager && $editLoanId > 0 && $_SERVER["REQUEST_METHOD"] !== "POST") {
    $editLoan = fetchTrainerLoanRecord($pdo, $editLoanId, $currentGameId);
    if ($editLoan) {
        $formData = [
            "id" => (int)$editLoan["id"],
            "trainer_id" => isset($trainerMap[(int)$editLoan["trainer_id"]]) ? (int)$editLoan["trainer_id"] : 0,
...
$records = fetchTrainerLoanRows($pdo, $currentGameId, $selectedDate, $selectedTrainerId);
$summary = summarizeTrainerLoanRows($records);

if (($_GET["export"] ?? "") === "xlsx") {
    try {
        exportTrainerLoansXlsx($records, $selectedDate, $currentGameName);
    } catch (Throwable $throwable) {
        logTrainerLoanException($throwable);
    ...
    <main class="main-content">
        ...
        <section class="trainers-summary-grid">
            ...
            <div class="card trainer-stat-card">
                ...
                <strong class="trainer-stat-value"><?php echo htmlspecialchars(formatTrainerLoanAmountWithCurrency($summary["total_amount"]), ENT_QUOTES, "UTF-8"); ?></strong>
            </div>
        </section>
                ...
                <div class="card attendance-filter-card">
                     <div class="card-head">
                        ...
                        <?php if ($formData["id"] > 0): ?>
                            <a href="trainer_loans.php?date=<?php echo urlencode($selectedDate); ?>" class="btn btn-soft">إلغاء</a>
                        <?php endif; ?>
                    </div>

                    <form method="POST" class="attendance-filter-form">
                        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION["trainer_loans_csrf_token"], ENT_QUOTES, "UTF-8"); ?>">
                        ...
                        <div class="attendance-filter-grid">
                            <div class="form-group">
                                <label for="loanTrainer">المدرب</label>
                                <select name="trainer_id" id="loanTrainer" required>
                ...
                <div class="card attendance-filter-card">
                    ...
                    <form method="GET" class="attendance-filter-form">
                        ...
                        <div class="attendance-filter-actions">
                            ...
                            <a href="trainer_loans.php?<?php echo htmlspecialchars($exportQuery, ENT_QUOTES, "UTF-8"); ?>" class="btn btn-warning">تصدير Excel</a>
                            <a href="trainer_loans.php" class="btn btn-soft">اليوم الحالي</a>
                        </div>
                    </form>
                </div>
            </div>
                                ...
                                <?php foreach ($records as $index => $record): ?>
                                    <tr>
                                        ...
                                        <td data-label="المبلغ">
                                            <span class="loan-amount-pill"><?php echo htmlspecialchars(formatTrainerLoanAmountWithCurrency($record["amount"] ?? 0), ENT_QUOTES, "UTF-8"); ?></span>
                                        </td>
                                        <td data-label="تاريخ السلفة"><?php echo htmlspecialchars((string)$record["loan_date"], ENT_QUOTES, "UTF-8"); ?></td>
                                        <td data-label="وقت التسجيل"><?php echo htmlspecialchars(formatTrainerLoanDateTimeValue($record["created_at"] ?? ""), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td data-label="أضيف بواسطة"><?php echo htmlspecialchars((string)($record["created_by_name"] ?? TRAINER_LOAN_EMPTY_VALUE), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td data-label="آخر تعديل بواسطة"><?php echo htmlspecialchars((string)($record["updated_by_name"] ?? TRAINER_LOAN_EMPTY_VALUE), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td data-label="الإجراءات">
                                            <?php if ($isManager): ?>
                                                <div class="inline-actions">
                                                    <a href="trainer_loans.php?date=<?php echo urlencode((string)$record["loan_date"]); ?>&trainer_id=<?php echo (int)$selectedTrainerId; ?>&edit=<?php echo (int)$record["id"]; ?>" class="btn btn-warning">تعديل</a>
