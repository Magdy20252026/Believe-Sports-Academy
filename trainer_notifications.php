...
const TRAINER_NOTIFICATION_EMPTY_VALUE = "—";

requireAuthenticatedUser();
requireMenuAccess("trainers-notifications");

function getTrainerNotificationTypes()
{
    return [
        "general" => "عام",
        "reminder" => "تذكير",
...
function getTrainerNotificationPriorities()
{
    return [
        "normal" => "عادي",
        "important" => "مهم",
...
function getTrainerNotificationStatuses()
{
    return [
        "visible" => "ظاهر",
        "hidden" => "مخفي",
...
function limitTrainerNotificationText($value, $maxLength)
{
    $value = (string)$value;
...
function normalizeTrainerNotificationTitle($title)
{
    $title = preg_replace('/\s+/u', ' ', trim((string)$title));
    return limitTrainerNotificationText($title, 160);
}

function normalizeTrainerNotificationMessage($message)
{
    $message = str_replace(["\r\n", "\r"], "\n", (string)$message);
    $message = preg_replace("/\n{3,}/", "\n\n", $message);
    $message = trim($message);
    return limitTrainerNotificationText($message, 3000);
}

function sanitizeTrainerNotificationChoice($value, array $allowedValues, $defaultValue)
{
...
function isValidTrainerNotificationDate($date)
{
...
function formatTrainerNotificationDateTimeValue($dateTimeString)
{
    ...
    if ($dateTimeString === "") {
        return TRAINER_NOTIFICATION_EMPTY_VALUE;
    }
    ...
    return formatTrainerNotificationEgyptDateTimeLabel($dateTime);
}

function ensureTrainerNotificationsTable(PDO $pdo)
{
    $pdo->exec(
    ...
    $requiredColumns = [
        "title" => "ALTER TABLE trainer_notifications ADD COLUMN title VARCHAR(160) NOT NULL AFTER game_id",
        "message" => "ALTER TABLE trainer_notifications ADD COLUMN message TEXT NOT NULL AFTER title",
        "notification_type" => "ALTER TABLE trainer_notifications ADD COLUMN notification_type VARCHAR(50) NOT NULL DEFAULT 'general' AFTER message",
        "priority_level" => "ALTER TABLE trainer_notifications ADD COLUMN priority_level VARCHAR(20) NOT NULL DEFAULT 'normal' AFTER notification_type",
        "visibility_status" => "ALTER TABLE trainer_notifications ADD COLUMN visibility_status VARCHAR(20) NOT NULL DEFAULT 'visible' AFTER priority_level",
        "display_date" => "ALTER TABLE trainer_notifications ADD COLUMN display_date DATE NOT NULL AFTER visibility_status",
        "created_by_user_id" => "ALTER TABLE trainer_notifications ADD COLUMN created_by_user_id INT(11) NULL DEFAULT NULL AFTER display_date",
        "updated_by_user_id" => "ALTER TABLE trainer_notifications ADD COLUMN updated_by_user_id INT(11) NULL DEFAULT NULL AFTER created_by_user_id",
    ];

    foreach ($requiredColumns as $columnName => $sql) {
        $columnStmt = $pdo->query("SHOW COLUMNS FROM trainer_notifications LIKE " . $pdo->quote($columnName));
        if (!$columnStmt->fetch()) {
            $pdo->exec($sql);
        }
    }
...
function fetchTrainerNotificationRecord(PDO $pdo, $notificationId, $gameId)
{
    $stmt = $pdo->prepare(
...
function fetchTrainerNotificationRows(PDO $pdo, $gameId, $searchQuery, $statusFilter, $typeFilter)
{
    $sql = "SELECT
            n.id,
            n.title,
            ...
            n.created_at,
            n.updated_at,
            COALESCE(created_user.username, " . $pdo->quote(TRAINER_NOTIFICATION_EMPTY_VALUE) . ") AS created_by_name,
            COALESCE(updated_user.username, " . $pdo->quote(TRAINER_NOTIFICATION_EMPTY_VALUE) . ") AS updated_by_name
        FROM trainer_notifications n
...
function fetchTrainerNotificationSummary(PDO $pdo, $gameId)
{
    $stmt = $pdo->prepare(
        "SELECT
...
function getTrainerNotificationStatusClass($status)
{
    if ($status === "visible") {
...
function getTrainerNotificationPriorityClass($priority)
{
    if ($priority === "urgent") {
...
function buildTrainerNotificationsPageUrl(array $params = [])
{
    $filteredParams = [];
    ...
    return "trainer_notifications.php" . ($query !== "" ? "?" . $query : "");
}

function logTrainerNotificationException(Throwable $throwable)
{
    error_log("Trainer notifications page error: " . $throwable->getMessage());
}

if (!isset($_SESSION["trainer_notifications_csrf_token"])) {
    $_SESSION["trainer_notifications_csrf_token"] = bin2hex(random_bytes(32));
}

ensureTrainerNotificationsTable($pdo);
...
$notificationTypes = getTrainerNotificationTypes();
$notificationPriorities = getTrainerNotificationPriorities();
$notificationStatuses = getTrainerNotificationStatuses();
...
$activeMenu = "trainers-notifications";
...
$egyptDateTimeLabel = formatTrainerNotificationEgyptDateTimeLabel($egyptNow);

$searchQuery = trim((string)($_GET["search"] ?? ""));
$searchQuery = limitTrainerNotificationText($searchQuery, 100);
$statusFilter = sanitizeTrainerNotificationChoice($_GET["status_filter"] ?? "all", array_merge(["all" => "الكل"], $notificationStatuses), "all");
$typeFilter = sanitizeTrainerNotificationChoice($_GET["type_filter"] ?? "all", array_merge(["all" => "الكل"], $notificationTypes), "all");
...
$flashSuccess = $_SESSION["trainer_notifications_success"] ?? "";
unset($_SESSION["trainer_notifications_success"]);
if ($flashSuccess !== "") {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!hash_equals($_SESSION["trainer_notifications_csrf_token"], $csrfToken)) {
        $error = "الطلب غير صالح.";
    } else {
        $action = $_POST["action"] ?? "";
        $redirectSearch = limitTrainerNotificationText(trim((string)($_POST["redirect_search"] ?? "")), 100);
        $redirectStatusFilter = sanitizeTrainerNotificationChoice($_POST["redirect_status_filter"] ?? "all", array_merge(["all" => "الكل"], $notificationStatuses), "all");
        $redirectTypeFilter = sanitizeTrainerNotificationChoice($_POST["redirect_type_filter"] ?? "all", array_merge(["all" => "الكل"], $notificationTypes), "all");

        if ($action === "save") {
            $formData = [
                "id" => (int)($_POST["notification_id"] ?? 0),
                "title" => normalizeTrainerNotificationTitle($_POST["title"] ?? ""),
                "message" => normalizeTrainerNotificationMessage($_POST["message"] ?? ""),
                "notification_type" => sanitizeTrainerNotificationChoice($_POST["notification_type"] ?? "general", $notificationTypes, "general"),
                "priority_level" => sanitizeTrainerNotificationChoice($_POST["priority_level"] ?? "normal", $notificationPriorities, "normal"),
                "visibility_status" => sanitizeTrainerNotificationChoice($_POST["visibility_status"] ?? "visible", $notificationStatuses, "visible"),
            ...
            } elseif (!isValidTrainerNotificationDate($formData["display_date"])) {
                $error = "تاريخ الظهور غير صحيح.";
            }

            if ($error === "") {
                $existingNotification = $formData["id"] > 0 ? fetchTrainerNotificationRecord($pdo, $formData["id"], $currentGameId) : null;
            ...
            if ($error === "") {
                try {
                    if ($formData["id"] > 0) {
                        ...
                        auditLogActivity($pdo, "update", "trainer_notifications", $formData["id"], "إشعارات المدربين", "تعديل إشعار: " . $formData["title"]);
                        $_SESSION["trainer_notifications_success"] = "تم تحديث الإشعار.";
                    } else {
                        $insertStmt = $pdo->prepare(
                            "INSERT INTO trainer_notifications (game_id, title, message, notification_type, priority_level, visibility_status, display_date, created_by_user_id, updated_by_user_id)
                             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
                        );
                        $insertStmt->execute([
                            $currentGameId,
                        ...
                        auditLogActivity($pdo, "create", "trainer_notifications", $newNotificationId, "إشعارات المدربين", "إضافة إشعار: " . $formData["title"]);
                        $_SESSION["trainer_notifications_success"] = "تم تسجيل الإشعار.";
                    }

                    header("Location: " . buildTrainerNotificationsPageUrl([
                    ...
                    logTrainerNotificationException($throwable);
        ...
        if ($action === "delete") {
            ...
            if ($notificationId <= 0) {
                ...
                $notificationToDelete = fetchTrainerNotificationRecord($pdo, $notificationId, $currentGameId);
                $deleteStmt = $pdo->prepare("DELETE FROM trainer_notifications WHERE id = ? AND game_id = ?");
                ...
                try {
                    ...
                    if ($deleteStmt->rowCount() === 0) {
                        ...
                        auditLogActivity($pdo, "delete", "trainer_notifications", $notificationId, "إشعارات المدربين", "حذف إشعار: " . (string)($notificationToDelete["title"] ?? ""));
                        $_SESSION["trainer_notifications_success"] = "تم حذف الإشعار.";
                        header("Location: " . buildTrainerNotificationsPageUrl([
                            "search" => $redirectSearch,
                            "status_filter" => $redirectStatusFilter,
                            "type_filter" => $redirectTypeFilter,
...
if ($editNotificationId > 0 && $_SERVER["REQUEST_METHOD"] !== "POST") {
    $editNotification = fetchTrainerNotificationRecord($pdo, $editNotificationId, $currentGameId);
    if ($editNotification) {
        $formData = [
            "id" => (int)$editNotification["id"],
            "title" => (string)($editNotification["title"] ?? ""),
            "message" => (string)($editNotification["message"] ?? ""),
            "notification_type" => sanitizeTrainerNotificationChoice($editNotification["notification_type"] ?? "general", $notificationTypes, "general"),
            "priority_level" => sanitizeTrainerNotificationChoice($editNotification["priority_level"] ?? "normal", $notificationPriorities, "normal"),
            "visibility_status" => sanitizeTrainerNotificationChoice($editNotification["visibility_status"] ?? "visible", $notificationStatuses, "visible"),
            "display_date" => (string)($editNotification["display_date"] ?? $todayDate),
        ];
    }
}

$summary = fetchTrainerNotificationSummary($pdo, $currentGameId);
$records = fetchTrainerNotificationRows($pdo, $currentGameId, $searchQuery, $statusFilter, $typeFilter);
$cancelUrl = buildTrainerNotificationsPageUrl([
    "search" => $searchQuery,
            ...
            <div class="attendance-stack">
                <div class="card attendance-filter-card">
                    ...
                    <form method="POST" class="attendance-filter-form">
                        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION["trainer_notifications_csrf_token"], ENT_QUOTES, "UTF-8"); ?>">
                ...
                <div class="card attendance-filter-card">
                    ...
                    <form method="GET" class="attendance-filter-form">
                        ...
                        <div class="attendance-filter-actions">
                            ...
                            <a href="trainer_notifications.php" class="btn btn-soft">إعادة ضبط</a>
                        </div>
                    </form>
                </div>
            </div>
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <?php foreach ($records as $index => $record): ?>
                                    <?php $editUrl = buildTrainerNotificationsPageUrl([
                                        "search" => $searchQuery,
                                        "status_filter" => $statusFilter,
                                        "type_filter" => $typeFilter,
                                        "edit" => (int)$record["id"],
                                    ...
                                    <tr>
                                        ...
                                        <td data-label="الأولوية">
                                            <span class="status-chip <?php echo htmlspecialchars(getTrainerNotificationPriorityClass((string)$record["priority_level"]), ENT_QUOTES, "UTF-8"); ?>"><?php echo htmlspecialchars($notificationPriorities[$record["priority_level"]] ?? (string)$record["priority_level"], ENT_QUOTES, "UTF-8"); ?></span>
                                        </td>
                                        <td data-label="الحالة">
                                            <span class="status-chip <?php echo htmlspecialchars(getTrainerNotificationStatusClass((string)$record["visibility_status"]), ENT_QUOTES, "UTF-8"); ?>"><?php echo htmlspecialchars($notificationStatuses[$record["visibility_status"]] ?? (string)$record["visibility_status"], ENT_QUOTES, "UTF-8"); ?></span>
                                        </td>
                                        <td data-label="تاريخ الظهور"><?php echo htmlspecialchars((string)$record["display_date"], ENT_QUOTES, "UTF-8"); ?></td>
                                        <td data-label="وقت التسجيل"><?php echo htmlspecialchars(formatTrainerNotificationDateTimeValue($record["created_at"] ?? ""), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td data-label="أضيف بواسطة"><?php echo htmlspecialchars((string)($record["created_by_name"] ?? TRAINER_NOTIFICATION_EMPTY_VALUE), ENT_QUOTES, "UTF-8"); ?></td>
                                        <td data-label="آخر تعديل بواسطة"><?php echo htmlspecialchars((string)($record["updated_by_name"] ?? TRAINER_NOTIFICATION_EMPTY_VALUE), ENT_QUOTES, "UTF-8"); ?></td>
