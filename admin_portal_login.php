...
try {
    $passwordColCheck = $pdo->query("SHOW COLUMNS FROM admins LIKE 'password'");
    if (!$passwordColCheck->fetch()) {
        $pdo->exec("ALTER TABLE admins ADD COLUMN password VARCHAR(255) NOT NULL DEFAULT '' AFTER salary");
    }
    ...
    if ($barcodeInput === "" || $passwordInput === "") {
        ...
        try {
            $adminStmt = $pdo->prepare(
                "SELECT a.id, a.name, a.phone, a.barcode, a.salary, a.game_id, a.password,
                        a.attendance_time, a.departure_time
                 FROM admins a
                 WHERE a.barcode = ?
                 LIMIT 1"
            );
            $adminStmt->execute([$barcodeInput]);
                ...
                if ($storedPassword === "") {
                    ...
                    if (!empty($passwordInfo["algo"])) {
                        ...
                        if ($passwordMatched && password_needs_rehash($storedPassword, PASSWORD_DEFAULT)) {
                            ...
                            if ($newHash !== false) {
                                $rehashStmt = $pdo->prepare("UPDATE admins SET password = ? WHERE id = ?");
                                $rehashStmt->execute([$newHash, $adminRow["id"]]);
                            }
                        }
                    } else {
                    ...
                    if (!$passwordMatched) {
                        ...
                        $_SESSION["admin_portal_logged_in"] = true;
                        $_SESSION["admin_portal_id"] = (int)$adminRow["id"];
                        $_SESSION["admin_portal_name"] = (string)$adminRow["name"];
                        $_SESSION["admin_portal_game_id"] = (int)$adminRow["game_id"];
                        $_SESSION["admin_portal_site_name"] = $siteName;
                        $_SESSION["admin_portal_site_logo"] = $siteLogo;
                        header("Location: admin_portal.php");
                        exit;
                    }
                }
...
<div class="portal-login-wrap">
    <div class="portal-login-card">
        <div class="portal-login-brand">
            ...
            <span class="portal-login-badge">🛡️ بوابة الإداريين</span>
        </div>
