...
function normalizeGroupTimeInputValue($submittedValue)
{
    if (is_array($submittedValue)) {
        $hour = trim((string)($submittedValue['hour'] ?? ''));
        $minute = trim((string)($submittedValue['minute'] ?? ''));
        $period = strtoupper(trim((string)($submittedValue['period'] ?? '')));
        ...
        return normalizeTrainingTimeValue(
    ...
    return normalizeTrainingTimeValue($submittedValue);
}
...
function normalizeGroupTrainingDayTimes(array $submittedTimes, array $selectedDayKeys)
{
    ...
    foreach ($selectedDayKeys as $dayKey) {
        ...
        $normalizedTime = normalizeGroupTimeInputValue($submittedTimes[$dayKey] ?? '');
        if ($normalizedTime !== '') {
...
function buildGroupTrainingDayTimesFormData(array $dayTimes)
{
    ...
    foreach ($dayTimes as $dayKey => $timeValue) {
        ...
        $formDayTimes[$dayKey] = formatTrainingTimeLabel($timeValue);
    }
...
function filterTrainersByAvailability(array $trainerSuggestions, array $trainerDaysOffByName, array $trainingDayKeys)
{
    ...
    if (count($trainingDayKeys) === 0) {
        return array_values($trainerSuggestions);
    }

    $availableTrainers = [];
    foreach ($trainerSuggestions as $trainerName) {
        ...
        $trainerDaysOff = $trainerDaysOffByName[$trainerName] ?? [];
        if (count(array_intersect($trainingDayKeys, $trainerDaysOff)) === 0) {
            $availableTrainers[] = $trainerName;
        }
    }

    return array_values($availableTrainers);
}
    ...
    if (!hash_equals($_SESSION["groups_csrf_token"], $csrfToken)) {
        ...
        if ($action === "save") {
            ...
            if ($formData["group_name"] === "") {
                ...
                $error = "يجب إدخال ميعاد صحيح بنظام 24 ساعة لكل يوم من أيام التمرين المحددة.";
...
<script id="groupTrainerAvailability" type="application/json"><?php echo json_encode($trainerDaysOffByName, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?></script>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const getAvailableTrainerNames = function () {
        return Object.keys(trainerAvailability).filter(function (trainerName) {
            const trainerDaysOff = Array.isArray(trainerAvailability[trainerName]) ? trainerAvailability[trainerName] : [];
            return selectedDays.every(function (dayKey) {
                return trainerDaysOff.indexOf(dayKey) === -1;
            });
    ...
    const renderTrainerOptions = function () {
        const availableTrainerNames = getAvailableTrainerNames();
        trainerSelectElements.forEach(function (select) {
            const previousValue = select.value;
            const placeholder = select.hasAttribute('required')
                ? (availableTrainerNames.length > 0 ? 'اختر المدرب' : 'لا يوجد مدرب متاح')
                : 'اختياري';
            ...
            availableTrainerNames.forEach(function (trainerName) {
            ...
            if (previousValue && availableTrainerNames.indexOf(previousValue) === -1) {
                select.value = '';
            }

            if (availableTrainerNames.length === 0) {
    ...
    const renderTrainingTimes = function () {
        ...
        selectedDays.forEach(function (dayKey) {
            const dayTime = selectedDayTimes[dayKey] || '';
            ...
            timeField.addEventListener('input', function () {
                selectedDayTimes[dayKey] = timeField.value;
            });
