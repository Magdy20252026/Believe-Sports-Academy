...
function normalizeTrainerTimeInputValue($time)
{
    $time = substr(trim((string)$time), 0, 5);
    return isValidTrainerTime($time) ? $time : "";
}
...
{
    $time = normalizeTrainerTimeInputValue($time);
    if ($time === "") {
        return "";
    }
...
function buildTrainerWeeklyScheduleFormData(array $storedSchedules, $defaultAttendanceTime = "", $defaultDepartureTime = "")
{
    ...
    foreach (TRAINER_DAY_OPTIONS as $dayKey => $_dayLabel) {
        ...
        $formSchedule[$dayKey] = [
            "attendance_time" => normalizeTrainerTimeInputValue($daySchedule["attendance_time"] ?? ""),
            "departure_time" => normalizeTrainerTimeInputValue($daySchedule["departure_time"] ?? ""),
        ];
    }
...
function normalizeTrainerWeeklyScheduleInput($scheduleInput)
{
    ...
    foreach (TRAINER_DAY_OPTIONS as $dayKey => $_dayLabel) {
        ...
        $normalizedSchedule[$dayKey] = [
            "attendance_time" => normalizeTrainerTimeInputValue($dayInput["attendance_time"] ?? ""),
            "departure_time" => normalizeTrainerTimeInputValue($dayInput["departure_time"] ?? ""),
        ];
    }
