...
أصبح النظام ينشئ الجداول الأساسية وجداول النظام تلقائياً عند أول تشغيل.

تم أيضاً تجهيز مستخدم افتراضي للدخول إلى لوحة النظام عند أول تشغيل فقط:

- اسم المستخدم: `admin`
- كلمة السر: `123456`
- الصلاحية: `مدير`
- يملك صلاحية الوصول إلى كل الفروع وكل الألعاب

إذا كانت قاعدة البيانات فارغة تماماً، سيتم أيضاً إنشاء لعبة افتراضية باسم `اللعبة الرئيسية` داخل كل فرع نشط حتى يمكن تسجيل الدخول مباشرة. وإذا تم حذف المستخدم الافتراضي `admin` لاحقاً فلن يعاد إنشاؤه تلقائياً.

يجب تغيير كلمة سر المستخدم الافتراضي بعد أول دخول لأسباب أمنية، والتطبيق لا يفرض ذلك تلقائياً حالياً.
...
كما تم تحسين التجربة لتشمل:

- تحديث تلقائي حي داخل صفحات البوابات الثلاث عند وصول إشعار جديد بدون تدخل المستخدم
- تنبيه مرئي داخل الصفحة قبل إعادة تحميل المحتوى تلقائيًا
- تحسين إشعارات أندرويد لتظهر باسم التطبيق وشعاره
- تقوية فحص الإشعارات في الخلفية لجميع النكهات الثلاث
- حفظ نسخة محلية تلقائيًا من الصفحات التي تمت زيارتها لفتحها لاحقًا بدون إنترنت
- عند عودة الإنترنت يتم تحديث الصفحة من البوابة الأصلية بدل النسخة المحلية
